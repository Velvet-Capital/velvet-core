  ...
  ITokenExclusionManager
{
  ...
  event UserClaimedToken(address indexed user, uint256 indexed claimedTill);
   ...
   * @notice Emits a {UserClaimedToken} event upon successful token claim.
   * @notice Reverts with {ErrorLibrary.InvalidAddress} if the provided user address is zero.
   * @notice Reverts with {ErrorLibrary.NoTokensRemoved} if there are less than two snapshots.
   */
  ...
  /**
   * @notice Helper function to update user's last claimed ID.

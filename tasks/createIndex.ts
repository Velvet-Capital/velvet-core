...
import { CREATE_PORTFOLIO } from "./task-names";
import { chainIdToAddresses } from "../scripts/networkVariables";

task(CREATE_PORTFOLIO, "Deploy Portfolio Contract ")
  .addParam("portfoliofactory", "The portfolioFactory's address")
  ...
  .setAction(async (taskArgs): Promise<Contract> => {
    ...
    const PortfolioFactory = hre.ethers.getContractFactory("PortfolioFactory");
    let portfolioFactory = (await PortfolioFactory).attach(
      taskArgs.portfolioFactory,
    );
    const portfolio = await portfolioFactory.createPortfolio(
      taskArgs.name,
      taskArgs.symbol,
      newSafeAddress,
      velvetSafeModule.address,
